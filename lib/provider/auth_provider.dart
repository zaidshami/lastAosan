...
class AuthProvider with ChangeNotifier {
  ...
  Future socialLogin(SocialLoginModel socialLogin, Function callback) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      print("the response isssssss : " + apiResponse.response.data.toString());
  ...
  Future socialLoginWithApple(SocialLoginModelApple socialLogin, Function callback) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await authRepo.socialLoginApple(socialLogin);
    _isLoading = false;
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      // print("tokennn"+ apiResponse.response.data.toString());
      Map map = apiResponse.response.data;
      String message = '';
      String token = '';
      String temporaryToken= '';
      ...
      String errorMessage;
      if (apiResponse.error is String) {
         print('eeeeee'+apiResponse.error.toString());
        errorMessage = apiResponse.error.toString();
      } else {
        ErrorResponse errorResponse = apiResponse.error;
         print(errorResponse.errors[0].message);

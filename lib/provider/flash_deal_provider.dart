...
class FlashDealProvider extends ChangeNotifier {
  ...
  Future<void> getMegaDealList(int id, bool reload, BuildContext context, bool notify) async {
    if (_flashDealList.length == 0 || reload) {
      ApiResponse apiResponse = await megaDealRepo.getFlashDeal(id);
      if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
        if (apiResponse.response.data.isNotEmpty) {
          _flashDeal = FlashDealModel.fromJson(apiResponse.response.data[0]);
...
///the old getMegaDealList that is not handling the data is empty
  /*Future<void> getMegaDealList(int id,bool reload, BuildContext context, bool notify) async {
    if (_flashDealList.length == 0 || reload) {
